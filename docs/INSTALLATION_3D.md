...
- Download **My-Travel-Diary-3D-1.0.0-rc5-arm64.apk** for Galaxy S23 Ultra and other ARM64 devices. **My-Travel-Diary-3D-1.0.0-rc5.apk** is the universal alternative for supported 32-bit ARM and x86 devices. Both use the same production signing key and release optimization; install only one.
...
Install either compatible RC5 APK directly over an earlier production 3D version. The application ID and signing key are unchanged and the version code increases to 6. Keep the installed app: uninstalling is unnecessary and would remove its local data. Existing journeys and terrain remain available.
