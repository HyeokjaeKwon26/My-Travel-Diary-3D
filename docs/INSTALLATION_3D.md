...
### A. 이동 기록을 JSON 파일로 저장하기

먼저 **Google 지도 → 프로필 사진 → 내 타임라인**에서 여행 날짜의 기록이 있는지 확인하세요. 그 기록이 있는 휴대폰에서 내보냅니다.

1. 휴대폰의 **설정** 앱을 엽니다.
2. **위치 → 위치 서비스 → 타임라인**을 선택합니다.
3. **타임라인 데이터 내보내기**를 누른 뒤 **계속**을 누릅니다.
4. 저장 위치로 **Download / 다운로드** 등 기억하기 쉬운 폴더를 고르고 **저장**합니다.
5. 내보내기 완료를 확인합니다. 나중에 앱에서 고를 파일은 이 **`.json` 파일**입니다. 내용을 직접 편집하거나 여행 날짜별로 잘라낼 필요는 없습니다.

이는 [Google의 Android 타임라인 내보내기 안내](https://support.google.com/maps/answer/6258979?co=GENIE.Platform%3DAndroid&hl=ko)를 따른 순서입니다. 메뉴가 다르면 해당 안내를 확인하고, 휴대폰 설정의 검색창에서 `타임라인`을 찾아보세요.

**준비하다 막혔을 때**

- **내보내기 메뉴가 안 보여요:** Google 지도를 최신 버전으로 업데이트하고, 기록이 있는 기기인지 확인하세요. Android Go에서는 타임라인을 지원하지 않습니다.
- **여행 날짜의 기록이 없어요:** 여행 당시 사용한 Google 계정과 휴대폰을 확인하세요. 기록되지 않았거나 삭제된 경로를 이 앱이 사진만으로 복구할 수는 없습니다. 기록이 없다면 먼저 예시 여행으로 체험하세요.
- **Google 지도에서 백업은 켰는데 파일이 없어요:** 타임라인 백업을 켜는 것과 앱에서 선택할 JSON을 내보내는 것은 별개입니다. 위의 **데이터 내보내기**를 실행해 파일을 저장하세요.
- **컴퓨터의 Google Takeout에서 받으면 되나요?** 처음 준비한다면 위의 휴대폰 내보내기 방법을 이용하세요. 가져오기에는 실제 여행 기록이 담긴 JSON 파일이 필요합니다. ZIP 압축파일이나 지도 화면 캡처를 선택하지 마세요.

### B. 여행 사진 준비하기

사진을 별도 폴더에 모으거나 압축할 필요는 없습니다. **여행 때 찍은 원본 사진을 앱을 사용할 휴대폰에 보관**하세요. Google 포토 등에서 보이더라도 클라우드에만 있는 사진이라면 먼저 기기에 내려받아야 합니다.

새 여행 화면에 **Photos Not Linked**가 보이면 **Enable**을 눌러 사진 접근을 허용합니다. Android가 일부 사진만 선택하도록 제안하면 여행에 쓸 사진을 포함해 주세요. **Partial Photo Access** 상태에서 사진을 더 허용하려면 **Change**를 누릅니다. 앱은 접근 가능한 사진의 촬영 시간과 위치 정보를 이용해 여행에 연결합니다. 메신저로 재전송한 사진은 이 정보가 달라지거나 없어질 수 있어 원본을 권장합니다.

### C. 준비한 파일로 여행 만들기

1. 첫 화면에서 **+ New Travel Story**를 누릅니다.
2. 사진 접근 요청이 나오면 여행에 사용할 사진에 대한 접근을 허용합니다. 일부 사진만 허용하면 그 사진들만 사용할 수 있습니다.
3. **Choose Timeline JSON File**을 누르고 파일 선택 화면의 **다운로드** 또는 앞서 저장한 폴더에서 **`.json` 파일**을 선택합니다.
